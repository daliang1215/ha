...
def discoveryDevice():
    ...
    for state in states:
        ...
        _LOGGER.info("friendly_name: %s", friendly_name)
        ...
        _LOGGER.info("deviceType: %s", deviceType)
        if deviceType is None:
            continue

        deviceName = guessDeviceName(entity_id, attributes, _places, _aliases)
        _LOGGER.info("devicename: %s", deviceName)
        ...
        _LOGGER.info("prop: %s", prop)
        _LOGGER.info("action: %s", prop)
        if prop is None:
            continue
...
def queryDevice(name, payload):
    ...
    _LOGGER.info("deviceId: %s", deviceId)
